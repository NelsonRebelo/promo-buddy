  ...
  const run = async () => {
    ...
    const worker = async () => {
      while (idx < rows.length && !cancelRef.current) {
        ...
        const row = rows[i];
        try {
          const { status, data } = await sendOfferPromotion(row.advert, row.promotion, normalizedUserUuid);
          allResults.push({
            advert: row.advert,
            ...
            <Card className="glass rounded-3xl border-white/75 lg:sticky lg:top-24 lg:h-fit">
              ...
              <CardContent className="flex flex-col items-center space-y-5 text-center">
                <div className="w-full space-y-2 text-left">
                  <Label htmlFor="offer-user-uuid" className="text-xs font-medium text-muted-foreground">
                    User UUID
                  </Label>
                  <Input
                    id="offer-user-uuid"
